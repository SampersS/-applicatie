...
const GetActivity = (req, res,beginDatum, eindDatum, sprong) => {//datums in yyyy-MM-dd
    configConnect(function(connection){
        const queryry = "select dag as 1steDag, FLOOR(DATEDIFF(dag,DATE(?)/?) as dagGroep, SUM(knt) as sknt,SUM(knb) as sknb,SUM(wnb) as swnb,SUM(wnu) as swnu from activiteit_tabel where dag<=DATE(?) and dag>=DATE(?) Group by dagGroep;"
